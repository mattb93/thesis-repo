...
class AvroRunner(val sc: org.apache.spark.SparkContext, val sqlContext: org.apache.spark.sql.SQLContext) {
	def run(r: Runnable, collectionIDs: Array[String]) {
		for(collectionID <- collectionIDs) {
			
			var collection = new AvroTweetCollection(collectionID, sc, sqlContext, collectionID)
			r.run(collection)
		}
	}
}
